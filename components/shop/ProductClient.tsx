...
export default function ProductClient({ product }: { product: Product }) {
  ...
  useEffect(() => {
    fetch('/api/wishlist')
      .then(r => r.json())
      .then(data => {
        if (data.items) setWished(data.items.some((i: any) => i.productId === product.id))
      })
      .catch(() => {})
  }, [product.id])
            ...
            <div className="lg:pt-4">
              <div className="flex items-start justify-between mb-4">
                ...
                <button onClick={handleWishlist} className="p-2 hover:scale-110 transition-transform">
                  <Heart size={22} className={wished ? 'fill-lams-dark text-lams-dark' : 'text-lams-lightgray'} />
                </button>
              </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {colors.map((color, idx) => (
                      <button
                        key={color}
                        onClick={() => { setSelectedColor(color); if (images[idx]) setImgIdx(idx) }}
                        title={color}
